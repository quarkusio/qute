...
Moreover, it's possible to inject a template located in the `META-INF/resources/` directory: 
...
<2> If there is no `Located#value()` provided the field name is used to locate the template. In this particular case, the container will attempt to locate a template with path `META-INF/resources/items.html`.
